...
export const useAuthStore = create<AuthState>((set) => ({
    ...
    logout: async () => {
        ...
        if (refreshToken) {
            try {
                ...
                useNotificationStore.getState().addNotification("Logout erfolgreich", "success")
            } catch (e) {
                set({error: "Fehler" + e})
            } finally {
                useNotificationStore.getState().stopLoading()
    ...
    refreshToken: async () => {
        ...
        try {
            ...
            localStorage.removeItem("jwt");
            localStorage.removeItem("refreshToken");
            set({token: null, currentUser: null, isAuthenticated: false, error: "Fehler" + e});
            useNotificationStore.getState().addNotification("Session abgelaufen, bitte erneut anmelden", "error");
            return null;
        } finally {
            useNotificationStore.getState().stopLoading();
        }
